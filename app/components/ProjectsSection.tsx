...
import { Server, Code, Layers, ShieldCheck, Terminal, Rocket } from "lucide-react";
  ...
  return (
    <section id="projects" className="py-20 theme-bg transition-colors relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pl-4 lg:pl-24">
        ...
        {/* Douglaspadilla Interactive Project Cards Grid with Scroll Reveal */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredProjects.map((project) => {
            const isHoveredOrActive = activeCardId === project.id;
            return (
              <RevealOnScroll key={project.id} delay={project.delay}>
                <div
                  onMouseEnter={() => setActiveCardId(project.id)}
                  onMouseLeave={() => setActiveCardId(null)}
                  onClick={() => setActiveCardId(activeCardId === project.id ? null : project.id)}
                  className="infolavelada-card corner-brackets-top corner-brackets-bottom card-spotlight flex flex-col justify-between group cursor-pointer min-h-[380px]"
                >
                  ...
                  {/* Expanded Active / Hover Overlay State */}
                  {isHoveredOrActive && (
